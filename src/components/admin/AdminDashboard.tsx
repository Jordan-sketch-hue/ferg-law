...
function NewBookingModal({ token, onClose, onCreated }: { token: string; onClose: () => void; onCreated: (a: Appointment) => void }) {
  ...
  const [result, setResult] = useState<{ ref: string; meetingUrl: string | null; whenLabel: string } | null>(null);
  ...
  async function handleCreate() {
    ...
    if (isNaN(startsAt.getTime())) { setErr("Invalid date/time."); return; }
    setBusy(true); setErr(null);
    const r = await fetch("/api/admin/bookings/create", {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-admin-token": token },
      body: JSON.stringify({ service, startsAt: startsAt.toISOString(), name: name.trim(), email: email.trim(), phone: phone.trim(), sendEmail }),
    });
    const d = await r.json() as { ok?: boolean; ref?: string; meetingUrl?: string | null; whenLabel?: string; error?: string };
    setBusy(false);
    if (!d.ok) { setErr(d.error ?? "Could not create booking."); return; }
    setResult({ ref: d.ref!, meetingUrl: d.meetingUrl ?? null, whenLabel: d.whenLabel! });
    // Synthesise a partial Appointment object so the parent list updates immediately
    onCreated({
      id: d.ref!,
  ...
  const modalOverlay: React.CSSProperties = { position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", zIndex: 9000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 };
  const modalBox: React.CSSProperties = { background: CREAM, borderRadius: 14, padding: "28px 24px", width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,.3)" };
  const inp: React.CSSProperties = { ...S.input, width: "100%", boxSizing: "border-box" as const };
  const lbl: React.CSSProperties = { display: "block", fontSize: ".78rem", fontWeight: 600, color: MUTED, marginBottom: 4, textTransform: "uppercase", letterSpacing: ".04em" };
    ...
    return (
      <div style={modalOverlay} onClick={onClose}>
        <div style={modalBox} onClick={e => e.stopPropagation()}>
          <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.4rem", color: GREEN, margin: "0 0 8px" }}>Booking Created</h3>
          <p style={{ fontSize: ".85rem", color: MUTED, marginBottom: 16 }}>{result.whenLabel} · Ref: <strong>{result.ref}</strong></p>
          {result.meetingUrl ? (
            <>
              <div style={{ background: "rgba(47,122,82,.08)", border: "1px solid rgba(47,122,82,.2)", borderRadius: 10, padding: "14px 16px", marginBottom: 12 }}>
                <p style={{ margin: "0 0 6px", fontSize: ".78rem", fontWeight: 700, color: GREEN, textTransform: "uppercase", letterSpacing: ".06em" }}>Meeting Link</p>
                <p style={{ margin: "0 0 10px", fontSize: ".82rem", wordBreak: "break-all", color: "#12201a" }}>{result.meetingUrl}</p>
                <div style={{ display: "flex", gap: 8 }}>
                  <button type="button" onClick={() => { void navigator.clipboard.writeText(result.meetingUrl!); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                    style={{ ...S.waBtn, background: GREEN, color: "#fff", flex: 1 }}>
                    {copied ? "Copied!" : "Copy link"}
                  </button>
                  <a href={`https://wa.me/?text=${encodeURIComponent(`Your Ferguson Law consultation link:\n${result.meetingUrl}`)}`} target="_blank" rel="noopener noreferrer"
                    style={{ ...S.waBtn, background: "#25D366", color: "#fff", textDecoration: "none", flex: 1, textAlign: "center" }}>
                    Send via WhatsApp
                  </a>
                </div>
              </div>
              <a href={result.meetingUrl} target="_blank" rel="noopener noreferrer"
                style={{ ...S.waBtn, background: "rgba(47,122,82,.1)", color: GREEN, textDecoration: "none", display: "inline-block", marginBottom: 12 }}>
                Join meeting
              </a>
            </>
          ) : (
            <p style={{ fontSize: ".85rem", color: "#a23b3b", marginBottom: 12 }}>No meeting link generated — use the Zoom link button in the bookings table.</p>
          )}
          <button type="button" onClick={onClose} style={{ ...S.waBtn, background: "rgba(16,42,30,.08)", color: "#12201a", width: "100%" }}>Close</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={modalOverlay} onClick={onClose}>
      <div style={modalBox} onClick={e => e.stopPropagation()}>
        <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.4rem", color: GREEN, margin: "0 0 20px" }}>New Booking</h3>
        {err && <div style={{ padding: "8px 12px", background: "rgba(162,59,59,.08)", color: "#a23b3b", fontSize: ".82rem", borderRadius: 8, marginBottom: 12 }}>{err}</div>}
        <div style={row}><label style={lbl}>Client name *</label><input style={inp} placeholder="Full name" value={name} onChange={e => setName(e.target.value)} /></div>
        <div style={row}><label style={lbl}>Email</label><input style={inp} type="email" placeholder="client@example.com" value={email} onChange={e => setEmail(e.target.value)} /></div>
        <div style={row}><label style={lbl}>Phone</label><input style={inp} placeholder="+1 876 000 0000" value={phone} onChange={e => setPhone(e.target.value)} /></div>
        <div style={row}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
          <div style={{ flex: 1 }}><label style={lbl}>Date *</label><input style={inp} type="date" value={date} onChange={e => setDate(e.target.value)} min={new Date().toISOString().slice(0, 10)} /></div>
          <div style={{ flex: 1 }}><label style={lbl}>Time</label><input style={inp} type="time" value={time} onChange={e => setTime(e.target.value)} step="900" /></div>
        </div>
        <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginBottom: 20, fontSize: ".85rem", color: "#12201a" }}>
          <input type="checkbox" checked={sendEmail} onChange={e => setSendEmail(e.target.checked)} style={{ width: 16, height: 16 }} />
          Email meeting link to client
        </label>
        <div style={{ display: "flex", gap: 10 }}>
          <button type="button" onClick={onClose} style={{ ...S.waBtn, flex: 1, background: "rgba(16,42,30,.08)", color: "#12201a" }}>Cancel</button>
          <button type="button" onClick={() => void handleCreate()} disabled={busy}
            style={{ ...S.waBtn, flex: 2, background: GREEN, color: "#fff", opacity: busy ? .7 : 1 }}>
            {busy ? "Creating…" : "Create booking & generate link"}
          </button>
        </div>
      </div>
    </div>
