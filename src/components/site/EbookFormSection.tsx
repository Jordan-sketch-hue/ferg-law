...
export default function EbookFormSection() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/ebook/register", {
        ...
        body: JSON.stringify({ ...form, financing_type: form.financing_type.join(", "), source: "fergusonlawja-buyers-guide" }),
      });
  ...
  return (
    <section id="get-guide" style={{ background: "linear-gradient(165deg,#0e2518 0%,#1a3828 100%)", padding: "4rem 1.5rem" }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: "1.5rem", marginTop: 4 }}>
                  {["Yes", "No"].map(opt => (
                    <label key={opt} style={radio}>
                      <input type="radio" name="bg_first_time_buyer" value={opt} checked={form.first_time_buyer === opt} onChange={() => set("first_time_buyer", opt)} required />
                      {opt}
                    </label>
                  ))}
                </div>
