  ...
  return (
    <section className="dash-section">
            ...
            {!!(l.media ?? []).length && (
              <div className="thumbs">
                {l.media.filter((m) => m.type === "image").slice(0, 4).map((m, i) => (
                  <img className="t" key={i} src={m.url} alt="" />
                ))}
              </div>
            )}
          </div>
      ...
      <div className="dir-form" style={{ margin: "16px 0 0", maxWidth: "none" }}>
        <h3 style={{ fontFamily: "var(--serif)", color: "var(--ink)", marginBottom: 12 }}>Add a listing</h3>
        <div className="dform-field">
          <label>Title</label>
          ...
          {!!(form.media ?? []).length && (
            <>
              <p style={{ fontSize: 12, color: "var(--muted)", margin: "8px 0 4px" }}>
                ★ = display image shown in directory. Click a photo to set it.
              </p>
              <div className="thumbs">
                {(form.media ?? []).map((m, i) =>
                  m.type === "image" ? (
