...
INSERT INTO public.fl_site_pages (slug, title) VALUES
  ('home','Home Page'),('about','About'),('services','Services'),
  ('cost-estimator','Cost Estimator'),('booking','Booking'),
  ('faq','FAQ'),('value-estimator','Property Value Estimator'),('global','Global / Shared')
...
export async function POST(req: NextRequest) {
  ...
  for (const [label, sql] of [["schema", MIGRATION_SQL], ["seed", SEED_SQL], ["rls", RLS_SQL]] as [string, string][]) {
    ...
    if (error) {
      // Try direct insert for seed (fallback if pg_execute doesn't exist)
      if (label === "seed") {
        // Parse and run page inserts individually
        const { error: e2 } = await supabase.from("fl_site_pages").upsert([
          { slug: "home", title: "Home Page" }, { slug: "about", title: "About" },
          { slug: "services", title: "Services" }, { slug: "cost-estimator", title: "Cost Estimator" },
          { slug: "booking", title: "Booking" }, { slug: "faq", title: "FAQ" },
          { slug: "value-estimator", title: "Property Value Estimator" }, { slug: "global", title: "Global / Shared" },
        ], { onConflict: "slug" });
        results.push(`seed pages: ${e2 ? e2.message : "ok"}`);
      } else {
